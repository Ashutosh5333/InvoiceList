    ...
    <div>
          <div className="border-2 rounded-lg mt-10   overflow-x-auto">
            <table className="table-auto w-full  ">
              <thead>
                <tr className="rounded-t-lg bg-[#000000] ">
                  {INVOICE_HEAD.map((header, index) => (
                    <th
                      key={index}
                      className="px-4 font-normal text-[#FFFFFF] text-[.65rem] sm:text-[.8rem] text-start py-4"
                    >
                      {header}
                    </th>
                  ))}
              ...
              <tbody className="  text-left   text-[#000000] w-[90%] m-auto">
                {INVOICE_ROWS.map((row, rowIndex) => (
                  <tr
                    key={rowIndex}
                    className="font-semibold  text-[.45rem] sm:text-[.6rem] bg-[#F8F8F8] px-8 py-8 "
                  >
